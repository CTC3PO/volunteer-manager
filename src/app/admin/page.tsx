...
import {
  ...
  Share2, Download, X, AlertCircle, FileText
} from "lucide-react";
...
export default function DashboardPage() {
  ...
  const [importState, setImportState] = useState<"idle" | "parsing" | "preview" | "importing" | "done" | "error">("idle");
  ...
  const handleFile = async (file: File) => {
    if (!activeRetreatId) {
      setImportError("Không xác định được khóa tu hiện tại. Vui lòng chọn lại khóa tu.");
      setImportState("error");
      return;
    }

    if (!file.name.endsWith(".csv")) {
      setImportError("Vui lòng chọn file CSV (từ Google Sheets).");
      return;
    }
    ...
    try {
      ...
      setImportError("Không thể đọc file. Hãy kiểm tra định dạng CSV.");
      setImportState("error");
    }
  };
  ...
  const handleCreateRetreat = (e: React.FormEvent) => {
    ...
    router.push("/admin/cai-dat-retreat?isNew=true");
          ...
          <div className="card animate-fade-in" style={{ padding: "24px 20px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h3 style={{ fontSize: 15, fontWeight: 700, color: "var(--text-primary)", margin: 0, display: "flex", alignItems: "center", gap: 8 }}>
                📥 Import Tình Nguyện Viên (Google Sheets CSV)
              </h3>
              <button 
                className="btn btn-ghost btn-sm" 
                onClick={() => { setShowImport(false); handleResetImport(); }}
            ...
            {/* Instructions */}
            <div className="card" style={{ marginBottom: 24, borderLeft: "3.5px solid var(--accent)", background: "var(--bg-muted)", padding: "16px 20px" }}>
              <h4 style={{ fontSize: 13.5, fontWeight: 700, color: "var(--text-primary)", margin: "0 0 10px 0" }}>
                📋 Hướng dẫn xuất file CSV từ Google Sheets
              </h4>
              <ol style={{ fontSize: 13, color: "var(--text-secondary)", lineHeight: 2.0, paddingLeft: 20, margin: 0 }}>
                <li>Mở trang <strong style={{ color: "var(--text-primary)" }}>Google Sheets</strong> chứa danh sách đăng ký của khóa tu</li>
                <li>Chọn menu <strong style={{ color: "var(--text-primary)" }}>Tệp (File) → Tải xuống (Download) → Giá trị phân tách bằng dấu phẩy (.csv)</strong></li>
                <li>Kéo file vừa tải xuống vào vùng nét đứt bên dưới, hoặc click để chọn file</li>
                <li>Hệ thống sẽ hiển thị bảng xem trước. Nhấn "Xác nhận" để import vào khóa tu: <strong>{activeRetreat.ten}</strong></li>
              </ol>
            </div>
            ...
            {importState === "idle" && (
              <div
                ...
                onDrop={onDrop}
                onClick={() => document.getElementById("csv-input-dashboard")?.click()}
                style={{
              ...
              >
                <input id="csv-input-dashboard" type="file" accept=".csv" style={{ display: "none" }} onChange={onFileInput} />
                <Upload size={36} color="var(--text-muted)" style={{ margin: "0 auto 16px" }} />
                <p style={{ fontSize: 15, fontWeight: 600, color: "var(--text-primary)", marginBottom: 6 }}>
                  Kéo thả file CSV vào đây
                </p>
                <p style={{ fontSize: 13, color: "var(--text-secondary)" }}>
                  hoặc <span style={{ color: "var(--accent)", textDecoration: "underline" }}>nhấn để chọn file</span>
                </p>
                <p style={{ fontSize: 11.5, color: "var(--text-muted)", marginTop: 10 }}>
                  Chỉ chấp nhận file .csv xuất từ Google Sheets
                </p>
              </div>
            )}
            ...
            {importState === "parsing" && (
              <div style={{ textAlign: "center", padding: "48px 20px", color: "var(--text-secondary)" }}>
                ...
                <p style={{ fontSize: 14 }}>Đang xử lý file <strong>{fileName}</strong>...</p>
              </div>
            )}

            {importState === "error" && (
              <div style={{ padding: 20, borderRadius: 12, background: "rgba(248,113,113,0.1)", border: "1px solid rgba(248,113,113,0.25)" }}>
                <div style={{ display: "flex", gap: 10, alignItems: "center", color: "var(--red)", fontSize: 14 }}>
                  <AlertCircle size={18} />
                  <strong>{importError}</strong>
                </div>
                <button className="btn btn-secondary btn-sm" onClick={handleResetImport} style={{ marginTop: 14, cursor: "pointer" }}>
                  Thử lại
                </button>
              </div>
            )}
                ...
                <div style={{ 
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      <p style={{ fontWeight: 700, color: "var(--text-primary)", margin: 0, fontSize: 13.5, wordBreak: "break-all" }}>{fileName}</p>
                      <p style={{ fontSize: 12.5, color: "var(--text-secondary)", margin: "2px 0 0 0" }}>
                        Tìm thấy <strong style={{ color: "var(--accent)" }}>{preview.length} TNV</strong>
                      </p>
                    </div>
                  </div>
                  <div style={{ display: "flex", gap: 8 }}>
                    <button className="btn btn-ghost btn-sm" onClick={handleResetImport} style={{ cursor: "pointer" }}>
                      Hủy
                    </button>
                    <button className="btn btn-primary btn-sm" onClick={handleImport} style={{ cursor: "pointer", gap: 6 }}>
                      <CheckCircle size={14} />
                      Import {preview.length} TNV
                    </button>
                  </div>
                </div>

                <div style={{ overflowX: "auto", borderRadius: 10, border: "1px solid var(--border)", maxHeight: 360 }}>
                  <table className="data-table" style={{ fontSize: 13 }}>
                    <thead>
                      <tr>
                        ...
                        <th style={{ padding: "10px 12px" }}>Ngày đến</th>
                        <th style={{ padding: "10px 12px" }}>Ngày rời</th>
                        <th style={{ padding: "10px 12px" }}>Quốc tịch</th>
                      </tr>
                    </thead>
                    <tbody>
                      {preview.map((v, i) => (
                        <tr key={i}>
                          ...
                          <td style={{ padding: "8px 12px", fontWeight: 600 }}>{v.hoTen || "—"}</td>
                          <td style={{ padding: "8px 12px" }}>{v.email || "—"}</td>
                          <td style={{ padding: "8px 12px" }}>{v.ngayDen || "—"}</td>
                          <td style={{ padding: "8px 12px" }}>{v.ngayRoi || "—"}</td>
                          <td style={{ padding: "8px 12px" }}>{v.quocTich || "—"}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            {importState === "importing" && (
              <div style={{ textAlign: "center", padding: "48px 20px", color: "var(--text-secondary)" }}>
                <div style={{ fontSize: 28, marginBottom: 12 }}>⏳</div>
                <p style={{ fontSize: 14 }}>Đang import {preview.length} TNV...</p>
              </div>
            )}

            {importState === "done" && (
              <div style={{ padding: "32px 20px", borderRadius: 12, background: "var(--accent-bg)", border: "1px solid var(--accent)", textAlign: "center" }}>
                ...
                <p style={{ color: "var(--text-secondary)", fontSize: 13.5, marginBottom: 18 }}>
                  Đã thêm thành công <strong style={{ color: "var(--green)" }}>{preview.length} tình nguyện viên</strong> vào khóa tu này.
                </p>
                <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
                  <button className="btn btn-secondary btn-sm" onClick={handleResetImport} style={{ cursor: "pointer" }}>
                    Import tiếp
                  </button>
                  <button 
          ...
          <div className="card text-center" style={{ padding: "48px 20px", display: "flex", flexDirection: "column", alignItems: "center", gap: 14 }}>
            ...
            <div style={{ display: "flex", gap: 12, marginTop: 10, width: "100%", maxWidth: 320, flexDirection: "column" }}>
              <button 
                ...
                className="btn btn-primary" 
                style={{ justifyContent: "center", borderRadius: "8px", padding: 12, cursor: "pointer", border: "none", display: "flex", alignItems: "center", gap: 8, fontSize: 14 }}
              >
                <Upload size={14} /> Import từ file CSV (Google Sheets)
              </button>
              <Link href="/admin/tinh-nguyen-vien/moi" className="btn btn-secondary" style={{ justifyContent: "center", borderRadius: "8px", padding: 12 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                <div className="card animate-fade-in animate-delay-2">
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    <Link href="/admin/tinh-nguyen-vien" className="btn btn-secondary" style={{ justifyContent: "flex-start", borderRadius: "8px" }}>
                      <Users size={14} /> Xem danh sách TNV
                    </Link>
                    <button 
                      onClick={() => setShowImport(true)} 
                      className="btn btn-secondary" 
                      style={{ justifyContent: "flex-start", borderRadius: "8px", background: "transparent", border: "1px solid var(--border)", width: "100%", cursor: "pointer", display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", fontSize: 13, color: "var(--text-secondary)" }}
                    >
                      <Upload size={14} /> Import từ Google Sheets (CSV)
                    </button>
                    <Link href="/admin/tinh-nguyen-vien/moi" className="btn btn-primary" style={{ justifyContent: "flex-start", borderRadius: "8px" }}>
                ...
                <div style={{
                  ...
                  {activeVolunteers.map((v) => {
                    ...
                    return (
                      <Link
                        key={v.id}
                        href={`/admin/tinh-nguyen-vien/${v.id}`}
